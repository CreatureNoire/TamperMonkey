...
(function () {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (response) {
                ...
                if (response.status === 200) {
                    ...
                    let texteLabel = doc.querySelector('span.label-success')?.textContent?.trim() || 'non trouvé';
                    if (texteLabel === 'non trouvé' && response.finalUrl?.includes('/Prm/AfficherPv/')) {
                        texteLabel = 'Terminé / PV';
                    }
                    ...
                    // ✅ Vérifier et cocher automatiquement si Terminé / PV
                    if (texteLabel === 'Terminé / PV') {
                        cocherCaseTerminee(taskCard);
                    }
    ...
    // 🟢 Nouvelle fonction : coche automatiquement la case quand "Terminé / PV"
    function cocherCaseTerminee(taskCard) {
        const svgCase = taskCard.querySelector('svg.css-130');
        if (svgCase) {
            console.log("[AutoCheck] Case cochée automatiquement car Terminé / PV détecté.");
            svgCase.dispatchEvent(new MouseEvent("click", { bubbles: true }));
        }
    }
