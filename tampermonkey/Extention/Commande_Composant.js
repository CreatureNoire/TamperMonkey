(function() {
    ...
    function fillInput() {
        // Only fill if we're on the correct form (check for "Nouvelle demande" element)
        const nouvelleDemandeElement = document.querySelector('div.appmagic-label-text[data-control-part="text"]');
        if (!nouvelleDemandeElement || nouvelleDemandeElement.textContent.trim() !== 'Nouvelle demande') {
            return;
        }
    ...
    if (unsafeWindow !== unsafeWindow.top) {
        const pageObserver = new unsafeWindow.MutationObserver(() => {
            // Check for the specific "Nouvelle demande" element
            const nouvelleDemandeElement = unsafeWindow.document.querySelector('div.appmagic-label-text[data-control-part="text"]');
            const foundCorrectForm = nouvelleDemandeElement && nouvelleDemandeElement.textContent.trim() === 'Nouvelle demande';
