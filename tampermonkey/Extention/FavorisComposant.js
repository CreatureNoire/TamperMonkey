...
// @grant        unsafeWindow
// @updateURL    https://raw.githubusercontent.com/CreatureNoire/TamperMonkey/refs/heads/master/tampermonkey/Extention/FavorisComposant.js
// @downloadURL  https://raw.githubusercontent.com/CreatureNoire/TamperMonkey/refs/heads/master/tampermonkey/Extention/FavorisComposant.js
// @run-at       document-end
...
(function() {
                ...
                ">
                    ...
                    ">
                        ...
                        " class="modern-scrollbar">
                            ...
                            <!-- Badge informatif pour le mode édition -->
                            <div id="edit-folder-info" style="
                                margin-bottom: 24px;
                                padding: 12px 16px;
                            ...
                            ">
                                ...
                                <div style="
                                    ...
                                    line-height: 1.4;
                                ">
                                    Le <strong>Repère</strong> et le <strong>Commentaire</strong> seront modifiés uniquement pour le dossier : 
                                    <span id="current-folder-name" style="
                                        display: block;
                                        margin-top: 6px;
                                        font-weight: 700;
                                        color: #ffc107;
                                    "></span>
                                    La <strong>Catégorie</strong> sera synchronisée dans tous les dossiers.
                                </div>
                            </div>
    ...
    // ===== NETTOYAGE ET REFRESH DES FAVORIS EXISTANTS =====
    function cleanExistingFavoris() {
        ...
        const cleanedFavoris = favoris.map(fav => {
            ...
            // 1. Nettoyer les espaces dans text et title
            if (fav.text && fav.text !== fav.text.trim()) {
                updated.text = fav.text.trim();
                needsUpdate = true;
                modified = true;
            }
            
            if (fav.title && fav.title !== fav.title.trim()) {
                updated.title = fav.title.trim();
                needsUpdate = true;
                modified = true;
            }
            
            // 2. S'assurer que title existe (utiliser text si absent)
            if (!updated.title && updated.text) {
                updated.title = updated.text;
                needsUpdate = true;
                modified = true;
            }
            
            // 3. S'assurer que tous les champs nécessaires existent
            if (updated.symbole === undefined) {
                updated.symbole = '';
                needsUpdate = true;
                modified = true;
            }
            ...
            if (updated.repere === undefined) {
                updated.repere = '';
                needsUpdate = true;
                modified = true;
            }
            
            if (updated.categorie === undefined) {
                updated.categorie = '';
                needsUpdate = true;
                modified = true;
            }
            
            if (updated.designationComposant === undefined) {
                updated.designationComposant = '';
                needsUpdate = true;
                modified = true;
            }
            
            // 4. S'assurer que timestamp existe
        ...
        return cleanedFavoris;
    }
    
    // ===== REFRESH/SYNCHRONISATION DE TOUS LES DOSSIERS =====
    function refreshAllFolders() {
        const favoris = loadFavoris();
        const existingFolders = GM_getValue('powerbi_folders', []);
        const hierarchy = getFoldersHierarchy();
        
        // Récupérer tous les dossiers uniques depuis les favoris
        const foldersFromFavoris = new Set();
        favoris.forEach(fav => {
            const folderKey = [fav.symbole, fav.designation].filter(Boolean).join(' - ') || 'Sans PRM associé';
            foldersFromFavoris.add(folderKey);
        });
        
        // Ajouter les dossiers manquants
        let addedFolders = 0;
        foldersFromFavoris.forEach(folderKey => {
            if (!existingFolders.includes(folderKey)) {
                existingFolders.push(folderKey);
                addedFolders++;
            }
            
            // Créer dans la hiérarchie si absent
            if (!hierarchy[folderKey]) {
                createFolderInHierarchy(folderKey, null);
            }
        });
        ...
        if (addedFolders > 0) {
            console.log(`✅ ${addedFolders} dossier(s) synchronisé(s)`);
        }
    ...
    const UPDATE_URL = 'https://raw.githubusercontent.com/CreatureNoire/TamperMonkey/refs/heads/master/tampermonkey/Extention/FavorisComposant.js';
    ...
    function checkForUpdates(showNoUpdateMessage = false) {
        ...
        GM_xmlhttpRequest({
            method: 'GET',
            url: UPDATE_URL + '?t=' + Date.now(), // Cache busting
            onload: function(response) {
                try {
                    const remoteCode = response.responseText;
                    
                    // Extraire la version du code distant
                    const versionMatch = remoteCode.match(/@version\s+(\d+\.\d+)/);
                    if (!versionMatch) {
                        console.error('❌ Impossible de trouver la version dans le script distant');
                        if (showNoUpdateMessage) {
                            alert('❌ Erreur lors de la vérification de la version distante.');
                        }
                        return;
                    }
                    
                    const remoteVersion = versionMatch[1];
                    console.log(`📦 Version actuelle: ${SCRIPT_VERSION}`);
                    console.log(`📦 Version disponible: ${remoteVersion}`);
                    
                    // Comparer les versions
                    if (remoteVersion > SCRIPT_VERSION) {
                        const message = `🎉 Nouvelle version disponible !\n\n` +
                            `Version actuelle : ${SCRIPT_VERSION}\n` +
                            `Nouvelle version : ${remoteVersion}\n\n` +
                            `Voulez-vous ouvrir la page de mise à jour ?\n\n` +
                            `Note : Tampermonkey installera automatiquement\n` +
                            `la nouvelle version si vous cliquez sur OK.`;
            ...
            onerror: function(error) {
                ...
                if (showNoUpdateMessage) {
                    alert('❌ Erreur réseau lors de la vérification des mises à jour.');
                }
            }
        });
    }
    ...
    function init() {
        ...
        // 1. Nettoyer et mettre à jour tous les favoris existants
        cleanExistingFavoris();
        
        // 2. Synchroniser tous les dossiers
        refreshAllFolders();
        ...
        const openModal = (initialValues = {}, editIndex = null) => {
            ...
            if (editIndex !== null) {
                ...
                saveBtn.dataset.currentFolder = [initialValues.symbole, initialValues.designation].filter(Boolean).join(' - ') || 'Sans PRM associé';
                ...
                // Afficher le badge informatif avec le nom du dossier actuel
                const editFolderInfo = document.getElementById('edit-folder-info');
                const currentFolderName = document.getElementById('current-folder-name');
                if (editFolderInfo && currentFolderName) {
                    editFolderInfo.style.display = 'block';
                    currentFolderName.textContent = saveBtn.dataset.currentFolder;
                }
                ...
                delete saveBtn.dataset.currentFolder;
                ...
                // Masquer le badge informatif en mode ajout
                const editFolderInfo = document.getElementById('edit-folder-info');
                if (editFolderInfo) {
                    editFolderInfo.style.display = 'none';
                }
            }
        ...
        saveBtn.addEventListener('click', () => {
            ...
            if (saveBtn.dataset.editIndex !== undefined) {
                ...
                const existingFolders = GM_getValue('powerbi_folders', []);
                const currentFolder = saveBtn.dataset.currentFolder; // Le dossier dont on édite les valeurs
                ...
                favoris.forEach((fav, idx) => {
                    if (fav.title === componentTitle || fav.text === componentTitle) {
                        const folderKey = [fav.symbole, fav.designation].filter(Boolean).join(' - ') || 'Sans PRM associé';
                        allExistingFolders.push({ folderKey, index: idx, fav });
                    }
                });
                ...
                selectedFolders.forEach(folder => {
                    ...
                    if (existingIndex !== -1) {
                        ...
                        foldersToUpdate.push({ folder, index: existingIndex, folderSymbole, folderDesignation, isCurrentFolder });
                    } else {
                ...
                foldersToUpdate.forEach(({ folderSymbole, folderDesignation, isCurrentFolder }) => {
                    ...
                    if (idx !== -1) {
                        // Si c'est le dossier actuellement édité, mettre à jour repère et commentaire
                        // Sinon, conserver les valeurs existantes
                        if (isCurrentFolder) {
                            favoris[idx] = {
                                ...favoris[idx],
                                text: text,
                            ...
                            // Mettre à jour seulement la catégorie (commune à tous les dossiers)
                            favoris[idx] = {
                                ...favoris[idx],
                                text: text,
                                title: componentTitle,
                ...
                // Ajouter le composant dans les nouveaux dossiers (avec les valeurs du formulaire)
                ...
                if (foldersToUpdate.length > 0) {
                    const updatedInCurrent = foldersToUpdate.filter(f => f.isCurrentFolder).length;
                    const updatedInOthers = foldersToUpdate.length - updatedInCurrent;
                    if (updatedInCurrent > 0) {
                        messages.push(`✏️ Repère et commentaire mis à jour dans le dossier actuel`);
                    }
                    if (updatedInOthers > 0) {
                        messages.push(`🔄 Catégorie synchronisée dans ${updatedInOthers} autre(s) dossier(s)`);
                    }
                }
