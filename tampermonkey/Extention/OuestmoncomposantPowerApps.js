...
(function() {
    ...
    if (isRuntimeApp) {
        ...
        window.addEventListener('message', (event) => {
            ...
            if (event.data.type === 'GET_FIELD_VALUES') {
                console.log('📩 Demande reçue de récupérer les valeurs');
                
                // Chercher les inputs dans l'iframe
                const allInputs = document.querySelectorAll('input[type="text"]');
                console.log(`📊 ${allInputs.length} input(s) trouvé(s) dans l'iframe`);
                
                // Filtrer par ID
                const targetInputs = Array.from(allInputs).filter(input => {
                    const id = input.id || '';
                    return id.includes('field-5__control') || id.includes('field-6__control');
                });
                
                console.log(`🎯 ${targetInputs.length} input(s) cible(s) trouvé(s)`);
                ...
                // Envoyer les valeurs à la page parent
                window.parent.postMessage({
                    type: 'FIELD_VALUES_RESPONSE',
                    values: values
                }, 'https://apps.powerapps.com');
                
                console.log('✅ Valeurs envoyées à la page parent:', values);
            }
            ...
            if (event.data.type === 'SET_FIELD2_VALUE') {
                ...
                // Chercher les champs
                const allInputs = document.querySelectorAll('input[type="text"]');
                const field2Input = Array.from(allInputs).find(input => {
                    const id = input.id || '';
                    return id.includes('field-2__control');
                });
                const field3Input = Array.from(allInputs).find(input => {
                    const id = input.id || '';
                    return id.includes('field-3__control');
                });
                
                if (!field2Input) {
                    console.error('❌ Champ field-2 introuvable');
                    window.parent.postMessage({
                ...
                console.log(`✅ Champ field-2 trouvé: ${field2Input.id}`);
                if (field3Input) {
                    console.log(`✅ Champ field-3 trouvé: ${field3Input.id}`);
                }
                
                // Fonction d'injection pour un champ
                ...
                setTimeout(() => {
                    console.log('✅ Valeurs injectées avec succès');
                    console.log('📊 field-2:', field2Input.value);
                    if (field3Input) console.log('📊 field-3:', field3Input.value);
        ...
        return; // Arrêter l'exécution ici pour l'iframe
    }
    ...
    // Création du bouton "Ajouter aux Favoris"
    const btnAddFavoris = document.createElement('button');
    btnAddFavoris.id = 'btn-add-favoris-gallery';
    btnAddFavoris.textContent = '⭐ Ajouter aux Favoris';
    document.body.appendChild(btnAddFavoris);

    // Création du bouton "Voir Favoris"
    const btnFavoris = document.createElement('button');
    btnFavoris.id = 'btn-favoris-gallery';
    btnFavoris.textContent = '📋 Mes Favoris';
    document.body.appendChild(btnFavoris);
    ...
    modal.innerHTML = `
        <div class="modal-content-favoris">
            <div class="sidebar-folders">
                <h3 style="margin-top: 0; color: #667eea;">📁 Dossiers</h3>
                <button class="btn-create-folder" id="btn-create-folder">➕ Nouveau dossier</button>
                ...
                <div class="folder-form" id="folder-form">
                    ...
                    <select id="input-parent-folder" style="width: calc(100% - 10px); padding: 10px; margin: 5px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; box-sizing: border-box;">
                        <option value="null">Sans Dossier (Sans parent)</option>
                    </select>
    ...
    function addToFavoris() {
        ...
        if (!iframe) {
            ...
            showNotification('❌ Erreur : iframe non trouvée', true);
            return;
        }
        ...
        showNotification('⏳ Récupération en cours...', false);
    }
    ...
    window.addEventListener('message', (event) => {
        ...
        if (event.data.type === 'FIELD_VALUES_RESPONSE') {
            console.log('📥 Réponse reçue de l\'iframe:', event.data.values);
            
            const values = event.data.values;
            
            if (values.length === 0) {
                console.log('❌ Aucune valeur trouvée');
                showNotification('❌ Aucun élément trouvé à ajouter', true);
                return;
            }
    ...
    function openEditModal(index) {
        ...
        counter.id = 'folder-counter';
        counter.textContent = '1 sélectionné';
        counter.style.cssText = 'background: #667eea; color: white; padding: 5px 10px; border-radius: 15px; font-size: 12px; font-weight: bold;';
        ...
        infoMulti.style.cssText = 'background: #fff3cd; padding: 10px; border-radius: 5px; margin-bottom: 10px; font-size: 12px; color: #856404; border: 1px solid #ffc107;';
        infoMulti.innerHTML = '💡 <strong>Sélection multiple :</strong> Cochez les dossiers où vous souhaitez déplacer/copier le favori';
        sidebar.appendChild(infoMulti);
        ...
        selectParent.innerHTML = '<option value="null">📁 Racine (Sans parent)</option>';
    ...
    function openAddModal(value) {
        ...
        const infoMulti = document.createElement('div');
        infoMulti.style.cssText = 'background: #fff3cd; padding: 10px; border-radius: 5px; margin-bottom: 10px; font-size: 12px; color: #856404; border: 1px solid #ffc107;';
        infoMulti.innerHTML = '💡 <strong>Sélection multiple :</strong> Cochez les dossiers où vous souhaitez ajouter le favori';
        sidebar.appendChild(infoMulti);
    ...
    function displayFavoris(searchTerm = '') {
        ...
        Object.keys(categories).sort().forEach(categorie => {
            ...
            categories[categorie].forEach(favori => {
                ...
                // Gérer les repères multiples (séparés par des virgules)
                if (favori.repere) {
                    const reperes = favori.repere.split(',').map(r => r.trim()).filter(r => r);
                    if (reperes.length > 0) {
                        displayText = `${displayText} 📍 ${reperes.join(' 📍 ')}`;
                    }
                }
                
                if (favori.commentaire) displayText = `${displayText}<br><small style="color: #666;">💬 ${favori.commentaire}</small>`;
                
                // Afficher le dossier si recherche globale
                if (searchTerm && favori.folder) {
                    const folder = dossiers.find(d => d.id === favori.folder);
                    if (folder) {
                        displayText = `<small style="color: #667eea;">📁 ${folder.name}</small><br>${displayText}`;
                    }
                } else if (searchTerm && !favori.folder) {
                    displayText = `<small style="color: #667eea;">Sans Dossier</small><br>${displayText}`;
                }
                
                li.innerHTML = `
                    <div class="favoris-item-text">${displayText}</div>
    ...
    function displayFolders() {
        ...
        // Ajouter "Sans Dossier" en premier
            ...
            childDossiers.forEach((dossier) => {
                ...
                div.innerHTML = `
                    ...
                    <div class="folder-actions">
                        <button class="btn-delete-folder" data-id="${dossier.id}">🗑️</button>
                    </div>
                `;
                
                // Clic sur le dossier pour l'ouvrir
        ...
        // Événements pour supprimer les dossiers
    ...
    function updateParentFolderSelect() {
        ...
        select.innerHTML = '<option value="null">Sans Dossier (Sans parent)</option>';
    ...
    btnAddFavoris.addEventListener('click', addToFavoris);
